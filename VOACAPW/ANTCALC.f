...
ccc      write(*,'('' alf='',a)') alf
      if(alf(1:10).eq.'AREA      ') methodX='A'    !  AREA mode
      if(alf(1:11).eq.'FREQUENCY $') then  !read frequency for area coverage
         read(alf,'(11x,11f6.3)') freqarea   !  VOACAP
      else if(alf(1:9).eq.'FREQUENCY') then  !read frequency for area coverage
         read(alf,'(10x,11f5.2)') freqarea   !  ICEPAC
ccc         write(*,'(''freq='',11f6.2)') freqarea
...
1        format(10x,f5.2,a1,3(f9.2,a1),2x,a)
      ...
      else if(alf(1:11).eq.'FREQUENCY $') then !read frequency for area coverage
         read(alf,'(11x,11f6.3)') freqarea   !  VOACAP
      else if(alf(1:9).eq.'FREQUENCY') then  !read frequency for area coverage
         read(alf,'(10x,11f5.2)') freqarea   !  ICEPAC
      else if(alf(1:9).eq.'FREQ     ') then  !read frequency for area coverage
...
2        format(30x,f7.2,a1,f10.2,a1,2x,a)
      ...
      else if(alf(1:10).eq.'EXECUTE   ') then
         iexecute=1          !  flag to make sure no ANTENNA cards after EXECUTE
         ...
         if(iexecute.ne.0) go to 910        !  cannot have ANTENNA card after EXECUTE
...
3        format(10x,4i5,f10.3,1x,a21,1x,f5.1,f10.4)
     ...
     +        form='formatted')
         ...
         if(methodX.ne.'A') go to 200          !  point-to-point
c*****************************************************************
...
45         call invcon(30,freq,elev,antgains(ifreq,ielev+1))
         else if(jant.ge.90) then      !  Harris modifications (point-to-point)
            offazim=ztaz(itr)-beam_main    !  off azimuth
            if(offazim.lt.0.) offazim=offazim+360.
            close(22)
...
94       format(2f5.0,2f7.2,2f10.5)
         do 95 ifreq=1,30
         do j=1,91     !  make sure format does not make **********
            if(antgains(ifreq,j).lt.-99.99) antgains(ifreq,j)=-99.99
         end do
...
901   format(' In ANTCALC, could not OPEN file=',a)
      go to 999
910   write(*,911)
911   format(' You cannot have an ANTENNA card after EXECUTE.',
     +       ' The software does not allow that.')
      stop 'Input deck error -- ANTENNA card after EXECUTE.'
999   continue
ccc      call antprint
      return
      END
...
c        is back inside the propagation program.
     ...
     +                  array(30,91,20),aeff(30,20)
      ...
      common /Careaant/ iarray360(360,91,2)
         integer*2 iarray360
cccccc      equivalence (array,iarray360)        !  for area coverage
ccc      write(72,1) (iarray360(1,i,1),i=1,10)
ccc1     format('iarray360=',10i7)
      return
      end
c---------------------------------------------------------------------
