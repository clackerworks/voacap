c--------------------------------------------------------------
c         winapp 80000,120000 !jw
c# anttyp90.for
C***********************************************************************
      PROGRAM anttyp90
*    +               (run_directory,mode)
c      include <windows.ins> !jw
C***********************************************************************
...
c      call ucase(run_directory,nch_run) !jw
c      mode=cmnam() !jw
      call GET_COMMAND_ARGUMENT(2, mode) !jw
		print *, "Mode arg = ", run
c****************************************************************
ccc      write(66,'('' run_directory='',a)') run_directory(1:nch_run)
ccc      write(66,'('' mode='',a)') mode
c****************************************************************
      iquiet=1 !jw changed this to 1 to prevent the windows routine running
      if(iquiet.eq.0) then
...
c         ier=set_default_window@(window_handle) !jw
      end if
c****************************************************************
      open(21,file=run_directory(1:nch_run)//'/anttyp90.dat', !jw
     +        status='old',err=900)
      rewind(21)
...
c***********************************************************************
999   continue
c      call destroy_window(window_handle) !jw
      END
c--------------------------------------------------------------
