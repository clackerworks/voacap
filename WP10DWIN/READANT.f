...
ccc      nch=len_trim(alf)
      if(nch.le.2 .or. alf(nch:nch).eq.'/') then  ! no file name = 0 dB gain
      ...
      open(lu,file=run_directory(1:nch_run-3)//'antennas/'//
     +        alf(1:nch),
...
c          Windows-95 cannot open SCRATCH files
         open(13,file=run_directory(1:nch_run)//'/type13.tmp',
...
c          Windows-95 cannot open SCRATCH files
         open(14,file=run_directory(1:nch_run)//'/type14.tmp',
...
ccc      call antinit2      !  initialize CCIR antenna routines
      return
900   write(*,901) ios,run_directory(1:nch_run-3)//'antennas/'//
     +             alf(1:nch)
